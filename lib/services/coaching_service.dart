...
Future<Map<String, dynamic>> getHabitCoaching(String type, int dayOfWeek) async {
  try {
    ...
    Coachings.sort((a, b) {
      ...
      int positionComparison = (a['position'] as int).compareTo(b['position'] as int);
      ...
      int aUpdated = (a['updatedAt'] as int); // Assuming 'updatedAt' is a timestamp stored as an integer (milliseconds)
    ...
    if (Coachings.isNotEmpty) {
      ...
      await _firestore
          .collection('coaching')
          .doc(snapshot.docs[0].id) // Assuming the first document corresponds to the coaching entry
          .update({
        'updatedAt': Coachings.first['updatedAt'], // Set the 'updated' field to the current time
      });
